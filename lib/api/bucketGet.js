...
function searchMongo(mongifiedSearch, corsHeaders, bucketName,
    listParams, authInfo, requestMaxKeys, encoding, log, callback) {
    log.debug('querying mongo with query', { mongifiedSearch });
    // VIANNEY TO IMPLEMENT
    return callMongoClientToSearch(bucketName, mongifiedSearch,
        (err, list) => {
            if (err) {
                log.info('error searching mongo', { error: err });
                return callback(err
...
function bucketGet(authInfo, request, log, callback) {
    ...
    if (params.search !== undefined) {
        validatedAst = validateSearchParams(params.search);
        if (validatedAst instanceof Error) {
            return callback(validatedAst);
        }
    }
    ...
    metadataValidateBucket(metadataValParams, log, (err, bucket) => {
        ...
        if (params.search !== undefined) {
            ...
            const start = listParams.marker;
            const limit = params.maxKeys;
            const mongifiedSearch = parseWhere(validatedAst);
            // FIXME start, limit
            return searchMongo(mongifiedSearch, corsHeaders, bucketName,
                listParams, authInfo, requestMaxKeys, encoding, log, callback);
        }
        ...
        (err, list) => {
            ...
            // VIANNEY: I ABSRACTED HANDLE RESULT SO IT COULD BE
            // USED FOR MONGO SEARCH TOO. I MIGHT HAVE MESSED SOMETHING UP.
